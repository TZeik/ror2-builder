...
const DraggableItem = ({ item, isMobile }: { item: (typeof ITEMS)[0]; isMobile: boolean }) => {
  ...
  return (
    <Tooltip.TooltipProvider>
      <Tooltip.Root open={isMobile ? tooltipOpen : undefined} onOpenChange={!isMobile ? undefined : setTooltipOpen}>
        <Tooltip.Trigger asChild>
          <div
            ref={ref}
            className={`relative ${isMobile ? 'w-12 h-12 p-1' : 'w-16 h-16 p-2'} ${rarityStyle.bg} border ${
              rarityStyle.border
            } rounded-lg transition-colors cursor-pointer flex flex-col items-center justify-center ${
              isDragging ? "opacity-50" : "opacity-100"
            }`}
            onClick={handleItemClick}
            onContextMenu={(e) => {
              e.preventDefault();
              if (!isMobile) addItem(item);
            }}
          >
            <div
          ...
          >
            ...
            {isMobile && (
              <button
                ...
                className="w-full mt-2 py-1 bg-orange-500 hover:bg-orange-600 rounded text-xs font-medium flex items-center justify-center gap-1"
              >
                <FiPlus size={14} />
                Add Item
              </button>
...
export default function ItemGrid() {
  ...
  return (
    <div className={`space-y-${isMobile ? '4' : '6'}`}>
      {/* Search and filters */}
      <div className={`flex flex-col ${isMobile ? 'gap-3' : 'sm:flex-row gap-4 justify-between mb-8'}`}>
        {/* Search bar */}
        <div className="relative flex max-w-md">
          ...
          {searchTerm && (
            ...
            >
              <FiX className="text-gray-400 hover:text-white" size={isMobile ? 16 : 18} />
            </button>
          )}
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {RARITY_ORDER.map((rarity) => (
            <button
              ...
              className={`px-${isMobile ? '2' : '3'} py-1 text-xs font-medium rounded-full flex items-center gap-1 transition-colors ${
      ...
      {filteredItems.length > 0 ? (
        ...
        <div className={`text-center py-8 text-gray-400 ${isMobile ? 'text-sm' : ''}`}>
